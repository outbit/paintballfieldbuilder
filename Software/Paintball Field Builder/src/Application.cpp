...
void PFBApplication::MakeURChange(bool undo)
{
    ...
    */
    ...
    while (!debug_undo.empty()) {
        if (debug_undo.top().action == UR_CREATE) {
            PFB_LOG("action: create");
        }
        if (debug_undo.top().action == UR_DELETE) {
            PFB_LOG("action: delete");
        }
        if (debug_undo.top().action == UR_CHANGE) {
            PFB_LOG("action: change");
        }
        if (debug_undo.top().action == UR_NONE) {
            PFB_LOG("action: none");
        }
    ...
    while (!debug_redo.empty()) {
        if (debug_redo.top().action == UR_CREATE) {
            PFB_LOG("action: create");
        }
        if (debug_redo.top().action == UR_DELETE) {
            PFB_LOG("action: delete");
        }
        if (debug_redo.top().action == UR_CHANGE) {
            PFB_LOG("action: change");
        }
        if (debug_redo.top().action == UR_NONE) {
            PFB_LOG("action: none");
        }
    ...
    */
    ...
    if ( true == undo ) {
        ...
        if (thestack->top().action == UR_CREATE) {
            theaction = UR_DELETE;
        }
        if (thestack->top().action == UR_DELETE) {
            theaction = UR_CREATE;
        }
    } else {
        // redo
